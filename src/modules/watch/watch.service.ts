...
export class WatchService extends BaseService<WatchUser> {
  ...
  async confirmRequest(req: ConfirmRequest) {
    ...
    this.watchGateway.server.emit(
      `request-confirmed-${watch_request.user_id}`,
      requestResposne,
    );
  ...
  async makeRequest(id: string) {
    ...
    const school = await this.user_repo.findOne({
      where: { school_id: watch.school_id, roles: Role.School },
    });
    ...
    securities.forEach((user) => {
      this.watchGateway.server.emit(`new-request-${user.id}`, requestResposne);
    });
    return request;
  }
